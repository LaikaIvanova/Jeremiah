...
const commands = [
    ...
    //     .addStringOption(option =>
    //         option.setName('user')
    //             .setDescription('Choose a user to get a quote from')
    //             .setRequired(true)
    //             .addChoices(
    //                 { name: 'Laika', value: 'laika' },
    //                 { name: 'Reliix', value: 'reliix' },
    //                 { name: 'Bandito Dorito', value: 'bandito_dorito' }
    //             )),
    ...
    // new SlashCommandBuilder()
    //     .setName('checktag')
    //     .setDescription('Check if a user is displaying this server\'s tag')
    //     .addUserOption(option =>
    //         option.setName('user')
    //             .setDescription('User to check server tag for')
    //             .setRequired(true))
    //     .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    
    // new SlashCommandBuilder()
    //     .setName('updatelevelboard')
    //     .setDescription('Force update the levelboard immediately')
    //     .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
];
...
client.on('interactionCreate', async interaction => {
    ...
    //         const quote = getRandomQuote(selectedUser);
            
    //         if (!quote) {
    //             await interaction.reply({ 
    //                 content: `❌ No quotes found for this user or the quotes file is missing.`, 
    //                 ephemeral: true 
    //             });
    //             return;
    //         }
    ...
    //         await interaction.reply({ 
    //             content: '❌ Error getting quote. Please try again.', 
    //             ephemeral: true 
    //         });
    //     }
    // }
    ...
    // else if (commandName === 'checktag') {
    //     try {
    //         const targetUser = interaction.options.getUser('user');
    //         const guildId = interaction.guild.id;
            
    //         // Fetch the user to get the most up-to-date server tag information
    //         const fetchedUser = await client.users.fetch(targetUser.id, { force: true });
            
    //         const hasTag = hasServerTag(fetchedUser, guildId);
    //         const multiplier = getServerTagMultiplier(fetchedUser, guildId);
    ...
    //         if (hasTag) {
    //             statusEmoji = '✅';
    //             statusText = `**${targetUser.username}** is displaying this server's tag!`;
    //         } else {
    //             statusEmoji = '❌';
    //             if (fetchedUser.primaryGuild) {
    //                 if (fetchedUser.primaryGuild.identityEnabled) {
    //                     statusText = `**${targetUser.username}** is displaying a different server's tag (\`${fetchedUser.primaryGuild.tag || 'Unknown'}\`).`;
    //                 } else {
    //                     statusText = `**${targetUser.username}** has a server tag set but is not displaying it.`;
    //                 }
    //             } else {
    //                 statusText = `**${targetUser.username}** doesn't have any server tag set.`;
    //             }
    //         }
            
    //         const embed = {
    //             color: hasTag ? 0x00ff00 : 0xff6b6b,
    //             title: `${statusEmoji} Server Tag Check`,
    //             description: statusText,
    //             fields: [
    //                 {
    //                     name: 'XP Multiplier',
    //                     value: `**${multiplier}x** ${multiplier > 1 ? '(Bonus active!)' : '(No bonus currently)'}`,
    //                     inline: true
    //                 }
    //             },
    //             footer: {
    //                 text: hasTag ? 
    //                     'This user will receive XP bonuses when the multiplier is enabled!' : 
    //                     'This user can set this server\'s tag in their profile for XP bonuses.'
    //             },
    //             timestamp: new Date().toISOString()
    //         };
            
    //         // Add additional info if the user has a tag
    //         if (fetchedUser.primaryGuild && fetchedUser.primaryGuild.tag) {
    //             embed.fields.push({
    //                 name: 'Current Tag',
    //                 value: `\`${fetchedUser.primaryGuild.tag}\``,
    //                 inline: true
    //             });
    //         }
    ...
    //     } catch (error) {
    //         console.error('Check tag command error:', error);
    //         await interaction.reply({ 
    //             content: '❌ Error checking server tag information. Please try again.', 
    //             ephemeral: true 
    //         });
    //     }
    // }
    
    // else if (commandName === 'updatelevelboard') {
    //     try {
    //         const guildId = interaction.guild.id;
    //         const serverData = getServerLevels(guildId);
            
    //         // Check if a levelboard exists for this server
    //         if (!serverData.levelboardChannelId || !serverData.levelboardMessageId) {
    //             await interaction.reply({ 
    //                 content: '❌ No levelboard found for this server. Use `/levelboard` to create one first.', 
    //                 ephemeral: true 
    //             });
    //             return;
    //         }
    ...
    //         try {
    //             // Fetch the levelboard channel and message
    //             const channel = await client.channels.fetch(serverData.levelboardChannelId);
    //             const levelboardMsg = await channel.messages.fetch(serverData.levelboardMessageId);
                
    //             // Update the levelboard with current data
    //             const updatedContent = await getLevelboardText(serverData, client);
    //             await levelboardMsg.edit(updatedContent);
                
    //             await interaction.reply({ 
    //                 content: '✅ Levelboard has been successfully updated!', 
    //                 ephemeral: true 
    //             });
                
    //         } catch (fetchError) {
    //             console.error('Error updating levelboard:', fetchError);
                
    //             // Clear the invalid levelboard reference
    //             serverData.levelboardChannelId = null;
    //             serverData.levelboardMessageId = null;
    //             saveServerLevels(guildId, serverData);
                
    //             await interaction.reply({ 
    //                 content: '❌ Could not find the existing levelboard message. It may have been deleted. Use `/levelboard` to create a new one.', 
    //                 ephemeral: true 
    //             });
    //         }
            
    //     } catch (error) {
    //         console.error('Update levelboard command error:', error);
    //         await interaction.reply({ 
    //             content: '❌ Error updating levelboard. Please try again.', 
    //             ephemeral: true 
    //         });
    //     }
    // }
    ...
    //         let leaderboard = '```\n📊 SERVER LEADERBOARD\n\n';
    //         leaderboard += 'Rank | Level | XP        | User\n';
    //         leaderboard += '-----|-------|-----------|------------------\n';
            
    //         sortedUsers.forEach(([userId, userData], index) => {
    //             const rank = (index + 1).toString().padStart(4);
    //             const level = userData.level.toString().padStart(5);
    //             const xp = Math.round(userData.xp).toLocaleString().padStart(9);
    //             const username = userData.username.length > 16 ? 
    //                 userData.username.substring(0, 13) + '...' : 
    //                 userData.username.padEnd(16);
                
    //             leaderboard += `${rank} | ${level} | ${xp} | ${username}\n`;
    //         });
    ...
    //         await interaction.reply({ 
    //             content: '❌ Error getting leaderboard. Please try again.', 
    //             ephemeral: true 
    //         });
    //     }
    // }
...
if (TOKEN === 'YOUR_BOT_TOKEN_HERE') {
    console.error('Please replace YOUR_BOT_TOKEN_HERE and YOUR_CLIENT_ID_HERE with your actual bot credentials!');
    process.exit(1);
}
